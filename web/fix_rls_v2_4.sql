-- FINAL STRICT RLS FIX (v2.4)
-- Includes "Infinite Recursion" fix (SECURITY DEFINER)
-- Includes "Safe UUID Casting" fix (Custom Function) to prevent 22P02 errors on invalid paths

-- =====================================================================================
-- 0. HELPER FUNCTIONS
-- =====================================================================================

-- Anti-Recursion check for roles
...
-- Safe Conversion to UUID (Returns NULL instead of error)
...
-- =====================================================================================
-- 4. STORAGE ACCESS (Robust Path Parsing with Safe Cast)
-- =====================================================================================
...
WITH CHECK (
    bucket_id = 'documents' 
    AND (
        -- Valid Deal Participant (Using Safe Cast)
        EXISTS (
            SELECT 1 FROM public.deal_participants dp
            JOIN public.participants p ON dp.participant_id = p.id
            -- try_cast_uuid returns NULL for invalid paths -> dp.deal_id = NULL is false -> Safe
...
DO $$
BEGIN
    RAISE NOTICE 'Strict RLS (v2.4 - Robust & Safe) Applied.';
END $$;
