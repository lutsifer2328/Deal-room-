    ...
    const filteredSuggestions = suggestions
        ...
        .filter((suggestion, index, self) =>
            index === self.findIndex((t) => (
                t.name === suggestion.name && t.source === suggestion.source
            ))
        )
            ...
            {isOpen && filteredSuggestions.length > 0 && (
                ...
                >
                    {filteredSuggestions.map((suggestion, index) => {
                        ...
                        // Prefer ID from DB, else fallback to name + index
                        const uniqueKey = `${suggestion.source}-${suggestion.standardDocumentId || suggestion.id || suggestion.name}-${index}`;
